...
out_features = 256
model = LinearModel(len(total_genes) * 2, 1).to(device)
pred = MLPPredictor(out_features).to(device)
...
for e in range(epoch+1,300):
    ...
    for i in Slideseq_g:
        ...
        for p_u, p_v, n_u, n_v in zip( pos_u, pos_v, neg_u, neg_v):
            ...
            pos_score = model(torch.cat((p_u, p_v), 1))
            neg_score = model(torch.cat((n_u, n_v), 1))
            pos_score, neg_score = torch.sigmoid(pos_score), torch.sigmoid(neg_score)
            #loss is W2
            loss = torch.sum((1 - pos_score) ** 2 + neg_score ** 2)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
    ...
    for i in Merfish_g:
        ...
        if not os.path.exists('/home/bineva/Merfish/' + name + '_total_genes.h5ad'):
            continue
    ...
    for i in Starmap_g:
        ...
        all_pos = np.array([])
        all_neg = np.array([])
        expression = pd.read_csv(Starmap_dir + i + 'raw_total_expression_pd.csv', index_col = 0)
        adata = scanpy.AnnData(expression.T)
        #normalize the data
        scanpy.pp.normalize_total(adata, target_sum=1e4)
        scanpy.pp.log1p(adata)
        expression = adata.to_df().T
        expression.index = expression.index.str.upper()
        expression = expression.reindex(total_genes)
    ...
    torch.save(model.state_dict(),'model_linear_' + str(e) + '.pt')
    torch.save(pred.state_dict(),'pred_linear_' + str(e) + '.pt')