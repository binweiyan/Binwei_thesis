...
class VAE(nn.Module):
    ...
    def reparameterize(self, mu, logvar):
        std = torch.exp(0.5*logvar) #std = e^(1/2logvar)
    ...
    def forward(self, x):
        ...
        z = self.reparameterize(mu, logvar) #z = mu + eps*std
        return self.decode(z), mu, logvar #sigmoid(fc4(h3)), mu, logvar
...
hidden_size = 256
latent_size = 64
model = VAE(feature_size, hidden_size, latent_size).to(device)
# model.load_state_dict(torch.load('model_hetro2.pt'))
pred = MLPPredictor(latent_size).to(device)
...
file = 'log_VAE_total.txt'
resume_True = True
epoch = 50
model_path = ''
if  resume_True:
    if gene_2_gene:
        model.load_state_dict(torch.load(model_path + 'model_VAE_' + str(epoch) + '.pt'))
        pred.load_state_dict(torch.load(model_path + 'pred_VAE_' + str(epoch) + '.pt'))
else:
    epoch = -1
...
for e in range(epoch+1,300):
    ...
    for i in Merfish_g:
        ...
        expression = expression[[str(j) for j in Merfish_groups[i]]]
        ...
        for p_u, p_v, n_u, n_v in zip( pos_u, pos_v, neg_u, neg_v):
            ...
            h_p_u_mu, h_p_u_logvar = model.encode(p_u.float().to(device))
            h_p_v_mu, h_p_v_logvar = model.encode(p_v.float().to(device))
            pos_score = pred(h_p_u_mu, h_p_v_mu)
            h_n_u_mu, h_n_u_logvar = model.encode(n_u.float().to(device))
            h_n_v_mu, h_n_v_logvar = model.encode(n_v.float().to(device))
            neg_score = pred(h_n_u_mu, h_n_v_mu)

            VAE_loss_p = VAE_loss(model.decode(model.reparameterize(h_p_u_mu, h_p_u_logvar)), p_u, h_p_u_mu, h_p_u_logvar)
            VAE_loss_n = VAE_loss(model.decode(model.reparameterize(h_n_u_mu, h_n_u_logvar)), n_u, h_n_u_mu, h_n_u_logvar)

            pos_score, neg_score = torch.sigmoid(pos_score), torch.sigmoid(neg_score)
            loss = torch.mean(neg_score - pos_score)
            loss = loss + VAE_loss_p + VAE_loss_n
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
    ...
    for i in Starmap_g:
        ...
        expression = expression.loc[total_genes]
        expression = expression[[str(j) for j in Starmap_groups[i]]]
        matrix = expression
        #sigmoid of the matrix
        # matrix = 1 / (1 + np.exp(-matrix))
        nonzero = matrix.values.nonzero()
    ...
    torch.save(model.state_dict(),'model_VAE_' + str(e) + '.pt')
    torch.save(pred.state_dict(),'pred_VAE_' + str(e) + '.pt')